...
> 세컨더리 인덱스의 2단계 접근은 JOIN 유무와 무관하게 항상 존재한다. 
> JOIN은 그 비용을 드라이버 row 수만큼 증폭시키는 상황일 뿐이다.
> 매칭 건수가 적거나, 커버링 인덱스거나, 버퍼 풀에 캐시된 경우엔 문제없다.

## 왜 세컨더리 인덱스는 PK만 저장하는가

두 가지 이유가 있다.

### 이유 1: 데이터 중복 vs 동기화 문제

"모든 인덱스 트리가 데이터를 들긴 무거워서"가 맞다. 근데 더 근본적인 문제가 있다.

세컨더리 인덱스가 실제 row 데이터를 직접 포인터로 가리킨다고 가정하자.

```
created_at 인덱스 리프:
[2024-01-01 | 디스크 주소 0x1A3F]  ← 실제 페이지 주소
[2024-01-01 | 디스크 주소 0x2B7C]
[2024-01-02 | 디스크 주소 0x3C9D]
```

이 구조에서 row가 이동하면?

```
InnoDB는 페이지 분할, 병합, 압축 등으로
row의 물리적 위치가 수시로 바뀐다.

row가 0x1A3F → 0x9FF2로 이동하면
→ 이 row를 가리키는 모든 세컨더리 인덱스를 찾아서 전부 갱신
→ 세컨더리 인덱스가 10개면 10개 전부 수정
→ 인덱스가 많을수록 쓰기 비용이 폭발적으로 증가
```
...
PK 값은 안 바뀜
→ 세컨더리 인덱스는 갱신 불필요
→ 클러스터드 인덱스만 새 위치를 알면 됨
```

### 이유 2: 클러스터드 인덱스가 항상 최신 위치를 안다

```
세컨더리 인덱스: PK 값 저장 (논리적 참조)
클러스터드 인덱스: 실제 데이터 (물리적 위치)

row 이동 발생
→ 클러스터드 인덱스만 갱신
→ 세컨더리 인덱스는 PK만 들고 클러스터드에 물어보면 됨
```

**간접 참조 한 단계를 추가해서 동기화 비용을 제거한 설계다.**
>세컨더리 인덱스: 랜덤 I/O 있지만, 매칭 건수 적으면 풀스캔보다 훨씬 빠름 
>커버링 인덱스: 매칭 건수가 많거나 고빈도 쿼리일 때 랜덤 I/O 자체를 제거 
>
>랜덤 I/O가 존재한다 ≠ 반드시 문제다 
>풀스캔보다 랜덤 I/O 횟수가 적으면 세컨더리로 충분하다
