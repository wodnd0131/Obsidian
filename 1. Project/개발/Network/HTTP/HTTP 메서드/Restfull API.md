...
Stateless면 어떤 서버 인스턴스도 요청을 처리할 수 있습니다. 
로드밸런서가 자유롭게 분산할 수 있고, 서버가 죽어도 다른 서버가 즉시 대체합니다.
...
| 4   | **GraphQL / gRPC**         | REST의 한계를 어떤 상황에서 다른 것으로 대체하는지 이해하려면 REST를 먼저 알아야 함 |
 ...
 RESTful하다는 건 무엇을 보장하는가

한 줄 요약 먼저:

> **"HTTP를 이미 아는 사람이라면, 이 API를 문서 없이도 어느 정도 예측할 수 있다"**

좀 더 구체적으로 분해하면:

|RESTful이 보장하는 것|실제 의미|
|---|---|
|**예측 가능성**|`GET /users/123`이 조회일 거라고 예상 가능|
|**HTTP 인프라 활용**|CDN 캐시, 모니터링, API Gateway가 메서드/상태코드 기반으로 동작|
|**클라이언트-서버 독립성**|인터페이스가 일관되면 한쪽이 바뀌어도 다른 쪽 영향 최소화|
|**확장성**|Stateless 덕분에 서버 수평 확장 가능|
